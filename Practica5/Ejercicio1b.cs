...
Procedure Puente is
	...
	Begin
		loop
			SELECT
      	when ((total >= 1) & (pedidoCamion'count = 0)) => Accept pedidoAuto() do // si hay al menos una unidad libre, permite el paso del auto.
        											total--;
                            End pedidoAuto();
        End when;
        when ((total >= 2) & (pedidoCamion'count = 0))  => Accept pedidoCamioneta() do // si hay dos unidades libres, permite el paso de la camioneta (y la otra condicion tambien da verdadero)
        											total:= total-2;
                            End pedidoCamioneta();
        End when;
